"""
Formation Pressure Analysis Module
...
class OverburdenStress:
    ...
    Calculates overburden pressure from density data.
    """
    
    @staticmethod
    def calculate_overburden(depths: list[float],
                           densities: list[float],
                           water_depth: float = 0.0) -> dict:
        """
        ...
        # Initialize arrays
        overburden_pressure = np.zeros_like(depths)
        overburden_gradient = np.zeros_like(depths)
        ...
        for i in range(len(depths)):
            if i == 0:
                # First point
                if water_depth > 0:
                    overburden_pressure[i] = water_pressure + 0.433 * densities[i] * (depths[i] - water_depth)
                else:
                    overburden_pressure[i] = 0.433 * densities[i] * depths[i]
            else:
                # Trapezoidal integration
                depth_interval = depths[i] - depths[i-1]
                avg_density = (densities[i] + densities[i-1]) / 2.0
                overburden_pressure[i] = overburden_pressure[i-1] + 0.433 * avg_density * depth_interval
            
            # Calculate gradient
            if depths[i] > 0:
                overburden_gradient[i] = overburden_pressure[i] / depths[i]
        ...
        return {
            ...
            'equivalent_mud_weight_ppg': (overburden_gradient / PhysicalConstants.HYDROSTATIC_GRADIENT).tolist()
        }
    
    @staticmethod
    def estimate_overburden_from_depth(depth: float,
                                      water_depth: float = 0.0,
                                      average_density: float = 2.31) -> dict:
        """
        ...
        emw = gradient / PhysicalConstants.HYDROSTATIC_GRADIENT if gradient > 0 else 0
...
class PorePressurePrediction:
    """
    Pore pressure prediction methods
    
    Implements various pore pressure prediction techniques.
    """
    
    @staticmethod
    def normal_compaction_trend(depth: float,
                               surface_porosity: float = 0.40,
                               compaction_constant: float = 0.0003) -> float:
        """
    ...
    @staticmethod
    def eatons_method(observed_parameter: float,
    ...
    @staticmethod
    def sonic_method(observed_dt: float,
                    normal_dt: float,
                    overburden_gradient: float,
                    normal_pressure_gradient: float = PhysicalConstants.NORMAL_PORE_PRESSURE_GRAD) -> dict:
        """
        ...
        return PorePressurePrediction.eatons_method(
    ...
    @staticmethod
    def resistivity_method(observed_resistivity: float,
    ...
    @staticmethod
    def d_exponent_method(rop: float,
                         rpm: float,
                         wob: float,
                         bit_diameter: float,
                         mud_weight: float,
                         normal_d_exp: float = 1.0) -> dict:
        """
    ...
    @staticmethod
    def equivalent_depth_method(observed_parameter: float,
                               depths: list[float],
                               normal_parameters: list[float],
                               normal_gradients: list[float]) -> dict:
        """
        ...
        depths = np.array(depths)
        normal_parameters = np.array(normal_parameters)
        normal_gradients = np.array(normal_gradients)
        
        # Interpolate to find equivalent depth
        if observed_parameter < normal_parameters[0] or observed_parameter > normal_parameters[-1]:
            return {'error': 'Observed parameter outside normal trend range'}
        
        equivalent_depth = np.interp(observed_parameter, normal_parameters, depths)
        equivalent_gradient = np.interp(equivalent_depth, depths, normal_gradients)
...
class FractureGradient:
    ...
    Calculates formation fracture gradients.
    """
    
    @staticmethod
    def matthews_kelly_method(depth: float,
                             overburden_gradient: float,
                             pore_pressure_gradient: float,
                             matrix_stress_coefficient: float = 0.33) -> dict:
        """
        ...
        frac_gradient = ((overburden_gradient - pore_pressure_gradient) * 
    ...
    @staticmethod
    def eatons_fracture_method(depth: float,
                              overburden_gradient: float,
                              pore_pressure_gradient: float,
                              poissons_ratio: float = 0.25) -> dict:
        """
        ...
        frac_gradient = coeffiPhysicalConstants.HYDROSTATIC_GRADIENT * (overburden_gradient - pore_pressure_gradient) + pore_pressure_gradient
    ...
    @staticmethod
    def hubert_willis_method(depth: float,
                            pore_pressure_gradient: float = PhysicalConstants.NORMAL_PORE_PRESSURE_GRAD) -> dict:
        """
    ...
    @staticmethod
    def leak_off_test_analysis(test_pressure: float,
                               depth: float,
...
class AbnormalPressureDetection:
    """
    Abnormal pressure detection and analysis
    
    Identifies and characterizes abnormal pressure zones.
    """
    
    @staticmethod
    def classify_pressure_regime(pore_pressure_gradient: float,
                                normal_gradient: float = 0.465) -> dict:
        """
    ...
    @staticmethod
    def pressure_transition_zone(depths: list[float],
        ...
        return {
            ...
            'max_change': round(max(abs(gradient_changes)), 4) if len(gradient_changes) > 0 else 0
        }
    
    @staticmethod
    def drilling_window_analysis(depth: float,
                                pore_pressure_gradient: float,
...
class GeopressureAnalysis:
    """
    Comprehensive geopressure analysis
    
    Integrates multiple methods for geopressure evaluation.
    """
    
    @staticmethod
    def multi_method_prediction(depth: float,
                               ...
                               overburden_gradient: float = 1.0) -> dict:
        """
        ...
        if sonic_dt and normal_sonic:
            sonic_pred = PorePressurePrediction.sonic_method(
                sonic_dt, normal_sonic, overburden_gradient)
            predictions['sonic'] = sonic_pred
        
        # Resistivity method
        if resistivity and normal_resistivity:
            res_pred = PorePressurePrediction.resistivity_method(
        ...
        if predictions:
            gradients = [p['pore_pressure_gradient_psi_ft'] for p in predictions.values()]
...
def complete_pressure_analysis(depth: float,
                               mud_weight: float,
                               overburden_gradient: float,
                               sonic_dt: Optional[float] = None,
                               normal_sonic: Optional[float] = None,
    ...
    """
    # Overburden
    ob_analysis = OverburdenStress.estimate_overburden_from_depth(
        depth, water_depth)
    
    # Pore pressure prediction
    pore_pred = None
    if sonic_dt and normal_sonic:
        pore_pred = PorePressurePrediction.sonic_method(
            sonic_dt, normal_sonic, overburden_gradient)
    
    # Assume normal pressure if no prediction available
    pore_gradient = pore_pred['pore_pressure_gradient_psi_ft'] if pore_pred else PhysicalConstants.NORMAL_PORE_PRESSURE_GRAD
    
    # Fracture gradient
    frac_pred = FractureGradient.matthews_kelly_method(
        depth, overburden_gradient, pore_gradient)
    
    # Pressure classification
    pressure_class = AbnormalPressureDetection.classify_pressure_regime(pore_gradient)
    
    # Drilling window
    drilling_window = AbnormalPressureDetection.drilling_window_analysis(
        depth, pore_gradient, frac_pred['fracture_gradient_psi_ft'], mud_weight)
    
    return {
        'depth_ft': depth,
        'overburden': ob_analysis,
        'pore_pressure': pore_pred if pore_pred else {'estimate': 'normal', 'gradient_psi_ft': PhysicalConstants.NORMAL_PORE_PRESSURE_GRAD},
