"""Rock mechanics calculations for petroleum geomechanics.

This module implements rock mechanics analysis including:
...
def static_to_dynamic_correction(
    dynamic_E: float,
    dynamic_nu: float,
...
def ucs_from_sonic(dt_compressional: float, correlation: str = "default") -> float:
    ...
    if dt_compressional <= 0:
        logger.warning("Invalid dt_compressional: %s", dt_compressional)
        return 0.0
    ...
    logger.debug(
        "UCS from sonic: dt=%.1f, correlation=%s, UCS=%.0f",
        dt_compressional,
        correlation,
        ucs,
    )
...
def ucs_from_porosity(porosity: float, lithology: str = "sandstone") -> float:
...
def tensile_strength_from_ucs(ucs: float, ratio: float = 0.1) -> float:
...
def calculate_vertical_stress(
    ...
    # Integrate density
    vertical_stress = np.zeros_like(depths_arr)

    for i in range(len(depths_arr)):
        if i == 0:
            vertical_stress[i] = 0.433 * densities_arr[i] * depths_arr[i]
        else:
            depth_interval = depths_arr[i] - depths_arr[i - 1]
            avg_density = (densities_arr[i] + densities_arr[i - 1]) / 2.0
            vertical_stress[i] = (
                vertical_stress[i - 1] + 0.433 * avg_density * depth_interval
            )

    gradients = vertical_stress / depths_arr
...
def estimate_horizontal_stress(
...
def calculate_wellbore_stresses(
    stress_state: StressState,
    wellbore_pressure: float,
    wellbore_geo: WellboreGeometry,
) -> dict:
    ...
    sigma_theta_max = (
        3 * stress_state.max_horizontal_stress
        - stress_state.min_horizontal_stress
        - wellbore_pressure
    )

    sigma_theta_min = (
        3 * stress_state.min_horizontal_stress
...
def collapse_pressure(
    ...
    Pc = (
        (3 * stress_state.max_horizontal_stress - stress_state.min_horizontal_stress)
        - Nφ
        * (
            stress_state.max_horizontal_stress
            - stress_state.pore_pressure
            + 2 * rock_props.cohesion / math.sqrt(Nφ)
        )
    ) / (2 * (Nφ - 1))
    ...
    # Convert to EMW
    emw_collapse = Pc_safe / (
        PhysicalConstants.HYDROSTATIC_GRADIENT * stress_state.depth
    )
...
def mud_weight_window(
    ...
    """
    # Collapse analysis
    collapse = collapse_pressure(stress_state, rock_props)

    # Fracture analysis
    fracture = fracture_pressure(stress_state, rock_props)
...
def complete_geomechanical_analysis(
    depth: float,
    ...
    vp = sonic_to_velocity(sonic_dt_compressional)
    vs = sonic_to_velocity(sonic_dt_shear)

    # Elastic properties
    E = calculate_youngs_modulus(vp, vs, bulk_density)
    nu = calculate_poissons_ratio(vp, vs)
    G = calculate_shear_modulus(vs, bulk_density)

    # Rock strength
    ucs = ucs_from_sonic(sonic_dt_compressional)
    tensile = tensile_strength_from_ucs(ucs)
    friction_angle = estimate_friction_angle("sandstone")
    cohesion = cohesion_from_ucs(ucs, friction_angle)

    # Stresses
    vertical_stress = 0.433 * bulk_density * depth
    horizontal_stress = estimate_horizontal_stress(
    ...
    # Stability analysis
    stability = mud_weight_window(stress_state, rock_props, mud_weight)
