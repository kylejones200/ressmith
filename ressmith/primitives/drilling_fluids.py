"""Drilling fluids calculations and rheology models.

This module implements drilling fluids engineering including:
...
def mud_hydrostatic_pressure(mud_weight: float, depth: float) -> float:
...
def increase_mud_weight(
    current_weight: float,
    current_volume: float,
    target_weight: float,
    material_sg: float = 4.2,
) -> dict:
    ...
    sacks_per_bbl = (
        1470 * (target_weight - current_weight) / (94 * material_sg - target_weight)
    )
    ...
    volume_increase = (
        total_sacks
        * PhysicalConstants.BARITE_SACK_VOLUME_GAL
...
def dilute_mud_weight(
    current_weight: float,
    current_volume: float,
    target_weight: float,
    diluent_weight: float = PhysicalConstants.WATER_DENSITY_LB_GAL,
) -> dict:
...
def bingham_plastic_model(rheology_data: RheologyData) -> dict:
    ...
    Returns:
        dict with PV and YP
    """
    pv = rheology_data.reading_600 - rheology_data.reading_300
    yp = rheology_data.reading_300 - pv
    av = rheology_data.reading_600 / 2
...
def power_law_model(rheology_data: RheologyData) -> dict:
    ...
    """
    if rheology_data.reading_300 == 0:
        return {"error": "Invalid reading at 300 RPM"}

    n = PhysicalConstants.POWER_LAW_EXPONENT_FACTOR * math.log10(
        rheology_data.reading_600 / rheology_data.reading_300
    )

    k = (
        PhysicalConstants.POWER_LAW_K_FACTOR
        * rheology_data.reading_600
        / (PhysicalConstants.POWER_LAW_SHEAR_RATE ** n)
    )
...
def herschel_bulkley_model(rheology_data: RheologyData) -> dict:
    ...
    """
    tau_0 = rheology_data.reading_3

    if (
        rheology_data.reading_300 - tau_0 > 0
        and rheology_data.reading_600 - tau_0 > 0
    ):
        n = math.log10(
            (rheology_data.reading_600 - tau_0)
            / (rheology_data.reading_300 - tau_0)
        ) / math.log10(1022 / 511)
    else:
        n = 1.0

    if n > 0:
        k = (rheology_data.reading_600 - tau_0) / (1022 ** n)
    else:
        k = 0
...
def gel_strength_analysis(gel_10sec: float, gel_10min: float) -> dict:
...
def surface_pressure(
    ...
    """
    dp_drillpipe = pressure_loss_pipe(
        flow_rate, pv, yp, dp_id, dp_length, mud_weight
    )

    dp_drillcollar = pressure_loss_pipe(
        flow_rate, pv, yp, dc_id, dc_length, mud_weight
    )

    dp_bit = bit_pressure_loss(flow_rate, mud_weight, nozzle_sizes)

    dp_annulus_dc = pressure_loss_annulus(
        flow_rate, pv, yp, hole_id, dc_od, dc_length, mud_weight
    )

    dp_annulus_dp = pressure_loss_annulus(
...
def calculate_ecd(
    ...
    ecd = static_mud_weight + (
        annular_pressure_loss / (PhysicalConstants.HYDROSTATIC_GRADIENT * tvd)
    )

    return ecd


def optimize_hydraulics(
    flow_rate: float,
    mud_weight: float,
    max_pressure: float,
    optimization_method: str = "max_bit_hp",
) -> dict:
    ...
    if optimization_method == "max_bit_hp":
        ...
        tfa = math.sqrt(
            (mud_weight * flow_rate ** 2) / (10858 * optimal_bit_pressure)
        )
    ...
    if optimization_method == "max_impact_force":
        ...
        impact_force = (
            flow_rate * math.sqrt(mud_weight * optimal_bit_pressure)
        ) / 1930
...
def drilled_solids_fraction(
    mud_weight: float,
    base_fluid_weight: float,
    barite_sg: float = 4.2,
    drilled_solids_sg: float = 2.6,
) -> float:
    ...
    Args:
        ...
        drilled_solids_sg: Drilled solids specific gravity
...
def dilution_rate_required(
    current_solids: float,
    target_solids: float,
    system_volume: float,
) -> dict:
    ...
    dilution_volume = system_volume * (
        (current_solids - target_solids) / (1 - target_solids)
    )
...
def polymer_concentration(product_weight: float, volume: float) -> float:
...
def lime_treatment(
    volume: float,
    target_lime_concentration: float = 1.0,
) -> dict:
...
def analyze_mud_system(
    ...
    """
    bp_model = bingham_plastic_model(rheology)
    pl_model = power_law_model(rheology)
    gel_analysis = gel_strength_analysis(gel_10sec, gel_10min)

    pressure_gradient = ppg_to_psi_per_ft(mud_weight)
