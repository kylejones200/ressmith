...
def composite_ipr(
    ...
    layer_bubble_points: list[float | None] | None = None,
) -> float:
...
def joshi_horizontal_ipr(
    ...
    # Use r_equiv for effective wellbore radius
    re_effective = r_equiv if L_half > beta * h_half else re_horizontal
    J = (
        0.00708
        * k_darcy
        * reservoir_thickness
        / (oil_viscosity * formation_volume_factor)
        * (1.0 / (np.log(re_effective / wellbore_radius) + skin))
    )
