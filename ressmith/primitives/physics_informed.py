...
class MaterialBalanceParams:
    ...
    p: float | None = None  # Current pressure
...
class MaterialBalanceDecline(BaseDeclineModel):
    ...
    def rate(self, t: np.ndarray, params: dict[str, float]) -> np.ndarray:
        ...
        from ressmith.primitives.pvt import calculate_pvt_properties
        ...
        for i, (t_i, p_i, Np_i) in enumerate(zip(t, p, Np)):
            if drive_mechanism == "solution_gas":
                ...
                _ = water_drive_material_balance(p_i, Np_i, mb_params)  # noqa: F841
                if i > 0:
                    dNp = Np_i - Np[i - 1]
                    dt = t_i - t[i - 1] if t_i > t[i - 1] else 1.0
                    rates[i] = dNp / dt if dt > 0 else 0.0
                ...
                _ = gas_cap_drive_material_balance(p_i, Np_i, mb_params)  # noqa: F841
                if i > 0:
                    dNp = Np_i - Np[i - 1]
                    dt = t_i - t[i - 1] if t_i > t[i - 1] else 1.0
                    rates[i] = dNp / dt if dt > 0 else 0.0
...
class PressureDeclineModel(BaseDeclineModel):
...
def material_balance_forecast(
    ...
    material_balance_params: MaterialBalanceParams | None = None,
    horizon: int = 12,
) -> pd.Series:
    ...
    if not fit_result.success:
        ...
        try:
            ...
            params = fit_arps_exponential(t_days, q_values)
            fit_result.params = {
                "qi": params["qi"],
                "di": params["di"],
                "D": params["di"],
...
def pressure_decline_forecast(
    ...
    production_data: pd.Series | None = None,
    horizon: int = 12,
