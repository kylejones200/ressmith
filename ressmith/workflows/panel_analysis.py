...
def eur_with_company_controls(
    ...
    **kwargs: Any,
    ...
    Args:
        ...
        company_col: Column name for company
        county_col: Column name for county
        model_name: Type of decline model (default: 'arps_hyperbolic')
        min_months: Minimum months of data required
        **kwargs: Additional parameters for estimate_eur
    ...
    for well_id, well_data in panel_df.groupby(well_id_col):
        ...
        try:
            eur_result = estimate_eur(well_df, model_name=model_name, **kwargs)
            eur_results.append(
                {
                    well_id_col: well_id,
                    "eur": eur_result.get("eur", 0.0),
                    **eur_result.get("params", {}),
                }
            )
        except Exception as e:
            logger.warning(f"Failed to calculate EUR for {well_id}: {e}")
    ...
    # Merge with company and county info
    if company_col in panel_df.columns or county_col in panel_df.columns:
        ...
        if "company" in eur_df.columns:
            n_wells = len(eur_df)
            n_companies = eur_df["company"].nunique()
            logger.info(
                f"EUR calculated for {n_wells} wells across {n_companies} companies"
            )
        if "county" in eur_df.columns:
            n_with_county = eur_df["county"].notna().sum()
            n_counties = eur_df["county"].nunique()
            logger.info(
